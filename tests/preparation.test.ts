...
test("presentation, coffee chat, interview, class, meeting, workout, and study rules are deterministic",()=>{const scenarios:[string,Partial<CalendarEvent>,string[]][]=[
  ...
  ["coffee_chat",{title:"Coffee Chat with Justin",attendees:["Justin"]},["review-person","conversation-questions","review-context","notebook"]],
...
test("virtual meeting prioritizes link and device setup without commute tasks",()=>{const value=ready({title:"Project Team Meeting",meetingUrl:"https://meet.google.com/abc",description:"Weekly agenda"});assert.equal(value.category,"virtual_meeting");const taskIds=buildPreparationTasks(value).map(task=>task.id);assert.deepEqual(taskIds,["meeting-link","av-check","charge-laptop","review-agenda","join-early"]);assert.ok(!taskIds.includes("confirm-location"))});
...
test("limited metadata stays small and high-confidence",()=>{const tasks=buildPreparationTasks(ready());assert.equal(tasks.length,3);assert.deepEqual(tasks.map(task=>task.id),["review-details","required-materials","check-location-or-link"])});
